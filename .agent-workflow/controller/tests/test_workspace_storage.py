...
class WorkspaceStorageTests(unittest.TestCase):
    ...
    def test_reserve_open_duplicate_and_release_use_exact_anchor(self) -> None:
        with WorkspaceStorage(self.root) as storage:
    ...
    def test_path_replacement_does_not_redirect_descriptor_operations(self) -> None:
        storage = WorkspaceStorage(self.root)
    ...
    def test_nonempty_or_wrong_identity_anchor_cannot_be_released(self) -> None:
        with WorkspaceStorage(self.root) as storage:
            anchor = storage.reserve("DOC-001", attempt=1)
            (self.root / anchor.name / "evidence").write_text("retain")

            with self.assertRaisesRegex(WorkspaceStorageError, "release failed"):
                storage.release_empty(anchor)
            forged = anchor.__class__(
                anchor.task_id, anchor.attempt, anchor.name,
                anchor.device, anchor.inode + 1,
            )
            with self.assertRaisesRegex(WorkspaceStorageError, "identity changed"):
    ...
    def test_invalid_task_attempt_root_and_permissions_are_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(WorkspaceStorageError, "permissions"):
            WorkspaceStorage(unsafe)
        with self.assertRaisesRegex(WorkspaceStorageError, "root name"):
            WorkspaceStorage(self.parent / "bad name")

    def test_partial_descriptor_open_failure_rolls_back_directory(self) -> None:
        with WorkspaceStorage(self.root) as storage:
            real_open = os.open

            def fail_child(path, flags, *args, **kwargs):
                if path == "doc-001-attempt-1":
