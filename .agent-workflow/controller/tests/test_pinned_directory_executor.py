...
from td_controller.effective_identity import EffectiveIdentity, EffectiveIdentityError
from td_controller.pinned_directory_executor import (
    PinnedDirectoryExecutor,
    PinnedDirectoryExecutorError,
)
from td_controller.review_runtime import MAX_INPUT_BYTES, SubprocessExecutor
...
class PinnedDirectoryExecutorTests(unittest.TestCase):
    ...
    def test_effective_identity_owns_directory_and_stays_stable(self) -> None:
        descriptor = os.open(
            self.root, os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW
        )
        actual = EffectiveIdentity(os.geteuid(), os.getegid())
        wrong_owner = EffectiveIdentity(actual.uid + 1, actual.gid)
        try:
            with patch(
                "td_controller.pinned_directory_executor.validate_effective_identity",
                return_value=wrong_owner,
            ):
                with self.assertRaisesRegex(
                    PinnedDirectoryExecutorError, "initialization"
                ):
                    PinnedDirectoryExecutor(descriptor=descriptor)
            with patch(
                "td_controller.pinned_directory_executor.validate_effective_identity",
                side_effect=(actual, wrong_owner),
            ):
                executor = PinnedDirectoryExecutor(descriptor=descriptor)
                try:
                    with self.assertRaisesRegex(
                        PinnedDirectoryExecutorError, "identity changed"
                    ):
                        executor.verify()
                finally:
                    executor.close()
    ...
    def test_privilege_transition_failure_is_normalized(self) -> None:
        descriptor = os.open(
            self.root, os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW
        )
        try:
            with patch(
                "td_controller.pinned_directory_executor.validate_effective_identity",
                side_effect=EffectiveIdentityError("secret-transition"),
            ):
                with self.assertRaises(PinnedDirectoryExecutorError) as raised:
                    PinnedDirectoryExecutor(descriptor=descriptor)
            self.assertNotIn("secret-transition", str(raised.exception))
