...
def task(task_id="ORCH-003D1B0J"):
...
class MutationPipelineTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        try:
            self.handle = WorkspaceIdentityHandle(
                "ORCH-003D1B0J", attempt=1, generation="0" * 32,
    ...
    def test_invalid_dependencies_are_rejected(self) -> None:
        with self.assertRaisesRegex(MutationPipelineError, "broker"):
            MutationPipeline(broker=object())
        with self.assertRaisesRegex(MutationPipelineError, "applier"):
