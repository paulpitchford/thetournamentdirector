...
class WorktreeAdminBindingTests(unittest.TestCase):
    ...
    def test_fifo_marker_and_backlink_fail_promptly(self) -> None:
        binding = self.repository.verify_worktree_admin_binding(
            descriptor=self.workspace_fd, expected_identity=self.identity
        )
        backlink = (
        ...
        backlink.unlink()
        backlink.write_text(f"{self.workspace_path}/.git\n", encoding="ascii")
        marker = self.workspace_path / ".git"
        marker.unlink()
