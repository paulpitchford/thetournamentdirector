...
class MutationPipeline:
    ...
    def __init__(
        ...
        if broker is not None and not callable(getattr(broker, "run", None)):
            raise MutationPipelineError("mutation broker is invalid")
        if applier is not None and not callable(getattr(applier, "apply", None)):
            raise MutationPipelineError("mutation applier is invalid")
        self._broker = broker or TextMutationBroker()
        self._applier = applier or AttestedTextReplacementApplier()
...
def run_local_probe() -> None:
    ...
    task_contract = parse_task(
        {
            "id": "ORCH-003D1B0J", "status": "APPROVED",
