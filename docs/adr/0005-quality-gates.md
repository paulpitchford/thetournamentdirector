...
remediation cycles quarantine the task. Routine R0-R2 work may auto-merge only
when all deterministic and independent review statuses pass; R3 changes and
escalations require explicit user approval.
...
- Metrics are review signals, not substitutes for domain invariants or human
  judgement.
- Changing a gate is an R3 task requiring explicit user/CODEOWNER approval.
